...
app.post("/students", (req, res) => {
    const q = "insert into students (`gender`,`course`,`degree`)  values (?,?,?)";
...
app.get("/students/get/:id", (req, res) => {
    ...
    connection.query(q,[studentsId], (error, data) => {
        if (error) {
            console.log(error);
            return res.status(500).send({ error: "error in data fetching" });
        }
...
app.put("/students/update/:id", (req, res) => {
    const studentsId = req.params.id;
    const q = "update students set `gender`=? , `course`=? , `degree`=? where id =? ";
